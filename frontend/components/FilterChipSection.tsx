...
export function FilterChipSection({
  ...
  return (
    <div className="border-t border-[#f0ebe3] pt-4 first:border-t-0 first:pt-0">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      >
        <h3 className="text-xs font-black uppercase tracking-wide text-slate-500">
          {title}
          {items.length ? (
            <span className="ml-1 font-semibold text-slate-400">({items.length})</span>
          ) : null}
        </h3>
        <ChevronDown
          size={16}
        ...
        <>
              ...
              {visible.map((it) => {
                ...
                return (
                  <button
                    ...
                    className={`max-w-full rounded-lg border px-2.5 py-1.5 text-left text-xs font-semibold leading-snug ${
                      active
          ...
          {rest > 0 ? (
            <button
              type="button"
              onClick={() => setMore((v) => !v)}
              className="mt-2 text-xs font-bold text-[var(--aloha-green)] hover:underline"
            >
