...
function exec(client, cmd, timeoutMs = 600000) {
  return new Promise((resolve, reject) => {
...
c.on("ready", () => {
  c.sftp(async (err, sftp) => {
    ...
    try {
      ...
      await exec(
        c,
        `cd ${remoteBase} && pm2 restart aloha-shop-api --update-env && cd frontend && npm run build && pm2 restart aloha-shop --update-env && sleep 2 && pm2 ls | head -12`
      );
      console.log("OK history table deploy");
      c.end();
    } catch (e) {
      console.error(e);
      c.end();
