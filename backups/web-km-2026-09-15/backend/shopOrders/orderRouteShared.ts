...
import { resolveShopSellPrice } from "../shopCatalog/webKm.js";
...
> {
  ...
  for (const d of details) {
    ...
    const sell = resolveShopSellPrice(doc);
    const gia = sell.gia > 0 ? sell.gia : publicPrice(doc);
    ...
    const discount =
      sell.dangKm && sell.giaGoc != null
        ? Math.max(0, Math.round(sell.giaGoc - sell.gia))
        : 0;
    out.push({
      ...d,
      productName:
