...
import { ProductPrice, ProductSaleBadge } from "@/components/ProductPrice";
...
export function ProductCard({
  ...
  const dangKm = liveDangKm != null ? liveDangKm : Boolean(product.dangKm);
  const giaGoc =
    liveGiaGoc != null
      ? liveGiaGoc
      : dangKm
    ...
    const r = add(
      {
        ...
        gia: displayGia,
        ton: displayTon,
        giaGoc: dangKm ? giaGoc : undefined,
        dangKm,
        phanTramGiam: dangKm ? phanTramGiam : undefined,
      },
      1
    );
    if (!r.ok) {
  ...
  const hasSaleBadge =
    ...
    Number(giaGoc) > 0 &&
    displayGia > 0 &&
    displayGia < Number(giaGoc);

  const stockBadge = soldOut ? (
    <span className="rounded-full bg-slate-800/90 px-1.5 py-0.5 text-[11px] font-bold leading-none uppercase tracking-wide text-white shadow-sm backdrop-blur-sm">
      Hết hàng
    </span>
  ) : lowStock ? (
    <span className="rounded-full bg-amber-500/95 px-1.5 py-0.5 text-[11px] font-bold leading-none text-white shadow-sm">
      Sắp hết
    </span>
  ) : null;
        ...
        >
          ...
          {hasSaleBadge ? (
            <>
              ...
              {stockBadge ? (
                <div className="absolute right-1.5 top-1.5 z-10">{stockBadge}</div>
              ) : null}
            </>
          ) : (
      ...
      <div className={`flex flex-1 flex-col ${shopee ? "gap-1.5 p-2.5 sm:p-3" : "gap-2 p-3 sm:p-3.5"}`}>
        ...
        <div className="mt-auto pt-0.5">
          <div className="flex flex-wrap items-baseline gap-x-1">
            ...
            {product.dvt ? (
              <span
                className={`font-semibold text-[var(--aloha-muted)] ${
      ...
      try {
        ...
        for (const r of rows) {
          ...
          next[ma] = {
            gia: Number(r.gia) || 0,
            ton: Number(r.ton) || 0,
            giaGoc: r.dangKm ? Number(r.giaGoc) || undefined : undefined,
            dangKm: Boolean(r.dangKm),
            phanTramGiam: r.dangKm
              ? Number(r.phanTramGiam) || undefined
              : undefined,
          };
        }
        setLiveMap(next);
      ...
      {products.map((p) => {
        ...
        return (
          <ProductCard
            ...
            liveGiaGoc={live != null ? live.giaGoc : undefined}
            liveDangKm={live != null ? live.dangKm : undefined}
            livePhanTramGiam={live != null ? live.phanTramGiam : undefined}
          />
        );
      })}
    </div>
