...
import { Eye, EyeOff, Loader2, Percent, RefreshCw, Search, X } from "lucide-react";
...
import {
  ProductWebKmEditor,
  type AdminProductKmRow,
} from "./ProductWebKmEditor";
import { WebKmBulkBar } from "./WebKmBulkBar";

type Row = AdminProductKmRow & {
  nhom: string;
  nhomPath: string;
  ton: number;
  hienThiWeb: boolean;
...
export function ShopWebProductsAdmin() {
  ...
  const [kmMa, setKmMa] = useState<string | null>(null);
  ...
  const patchRow = (ma: string, patch: Partial<Row>) => {
    const safe: Partial<Row> = { ...patch };
    // Giữ giá cũ nếu patch mang 0/undefined (tránh hiện 0đ giả).
    if (!(Number(safe.gia) > 0)) delete safe.gia;
    if (!(Number(safe.giaWeb) > 0)) delete safe.giaWeb;
    if (!(Number(safe.giaThamChieu) > 0)) delete safe.giaThamChieu;
    setRows((list) => list.map((x) => (x.ma === ma ? { ...x, ...safe } : x)));
    setSuggestions((list) =>
      list.map((x) => (x.ma === ma ? { ...x, ...safe } : x))
    );
  };
        ...
        <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[12px] text-gray-500">
              Hiện/ẩn · Ghim · Nhãn tay · Khuyến mãi. «Tự động» = chưa gắn nhãn; shop có thể
              vẫn hiện «Bán chạy» khi tồn &gt; 20.
            </p>
          </div>
          ...
          <table className="w-full min-w-[1080px] text-left text-sm">
            <thead className="sticky top-0 z-[1] border-b border-gray-100 bg-[#3D6B3A] text-[11px] font-semibold uppercase tracking-wide text-white">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-semibold">Giá web</th>
                <th className="px-4 py-3 text-right font-semibold">Giá khuyến mãi</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="px-4 py-20 text-center text-[13px] text-gray-400">
                ...
                rows.map((row) => (
                  <FragmentRow
                    ...
                    busy={busyMa === row.ma}
                    onToggleKm={() =>
                      setKmMa((cur) => (cur === row.ma ? null : row.ma))
                    }
                    onCloseKm={() => setKmMa(null)}
                    onPatch={(patch) => patchRow(row.ma, patch)}
                    onToggle={() => void toggle(row)}
                    onSaveMerch={(patch) => void saveMerch(row, patch)}
                    onSavePinBlur={(n, saved) => {
...
function FragmentRow({
  ...
  onToggle: () => void;
  onSaveMerch: (patch: { webPin?: number; webBadge?: string }) => void;
  onSavePinBlur: (n: number, saved: string | undefined) => void;
}) {
  return (
    <>
      <tr className="border-b border-gray-50 transition hover:bg-gray-50/80">
        ...
        <td className="px-4 py-3 text-right text-[13px] font-medium tabular-nums text-gray-800">
          {fmtVnd(Number(row.giaWeb) > 0 ? Number(row.giaWeb) : row.gia)}
        </td>
        <td className="px-4 py-3 text-right text-[13px] font-medium tabular-nums text-gray-800">
          {row.dangKm ? (
            <>
              <div className="text-red-700">{fmtVnd(row.gia)}</div>
            ...
            onBlur={(e) => {
              ...
              const n =
                raw === ""
                  ? 0
                  : Math.max(0, Math.min(9999, Math.round(Number(raw) || 0)));
              onSavePinBlur(n, e.currentTarget.dataset.saved);
            }}
          />
        </td>
        <td className="px-3 py-3 text-center">
