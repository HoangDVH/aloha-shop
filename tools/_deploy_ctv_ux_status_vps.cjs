/**
 * Upload CTV portal UX status labels → VPS, rebuild FE, restart API + shop.
 */
const fs = require("fs");
...
const files = [
  ...
  "frontend/components/ctv-portal/shared/format.ts",
  "frontend/components/ctv-portal/types.ts",
...
curl -s -o /dev/null -w "ctv=%{http_code}\\n" "http://127.0.0.1:3002/ctv" || true
pm2 ls | head -20
echo DEPLOY_OK
`;
...
console.log("Deploying CTV UX status →", connect.host);
const c = new Client();
c.on("ready", () => {
